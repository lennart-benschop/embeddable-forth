...
INCLUDE examples/turtle.fs

FORGET GRPDEMO VARIABLE GRPDEMO

: PAUSE REDRAW 50 0 DO 
  KEY? IF 
    KEY 27 = IF 
       -1 THROW 
    THEN LEAVE THEN 
...
\G Like FORWARD, but draw a thick line
   ...
   X1 @ Y1 @ MOVETO X2 @ Y2 @  X4 @ Y4 @ TRIANGLE
   X2 @ Y1 @ X3 @ Y3 @ TRIANGLE POSX @ POSY @ COORD-TRANSLATE MOVETO 
  ;
...
: LEAF
    LEAF-SIZE IF
	2 GC
	8 0 DO 45 RT LEAF-SIZE FD LOOP 3 GC
    THEN ;
...
: RUN-DEMO
  GRAPHICS-MODE \ Tests require mode 0.
  BEGIN 
    POLYGON-CIRCLE-DEMO
    POLYGON-SPIRAL-DEMO
    LISSAJOUS-DEMO
