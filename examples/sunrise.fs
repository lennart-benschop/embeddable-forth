...
: YEAR-GRAPH ( year --- )
    ...
    365 OVER 3 AND 0= IF 1+ THEN >R 
    0 0 MOVETO
    S" Sunrise and Sunset "  PLOTTEXT
    LOCATION-NAME @ COUNT PLOTTEXT
    DUP 0 <# # # # # BL HOLD #> PLOTTEXT
    7 SETFG-G
    25 0 DO
        
	50 20 I 15 * + DUP >R MOVETO 600 R@ LINETO 20 R> 2 - MOVETO
	I 0 <# # # #> PLOTTEXT
    LOOP
    50 MONTH-X !
    13 0 DO
	MONTH-X @ DUP >R 20 MOVETO R> 380 LINETO
	I 12 < IF 
	    MONTH-X @ 11 MOVETO I 1+ >MONTH-NAME DROP 3 PLOTTEXT 
	    I 1+ OVER MONTH-LENGTH 550 365 */ MONTH-X +!
	THEN
    LOOP
    1 1 ROT >JULIAN2000
    R> 0 DO
	DUP I +  CALC-SUNRISE-SUNSET
	14 SETFG-G
	50 I 550 365 */ + 20 ROT 4 / + PLOTDOT
	10 SETFG-G
	50 I 550 365 */ + 20 ROT 4 / + PLOTDOT
    LOOP	
    DROP REDRAW TEXT-MODE
;
[THEN]
